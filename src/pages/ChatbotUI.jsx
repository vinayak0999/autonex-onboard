...
export default function ChatbotUI() {
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      if (!response.ok) {
        // This block will now correctly get the error message from FastAPI
      ...
      // --- THIS IS THE FIX ---
      // This block will now correctly display the error message (e.g., "Service Unavailable...")
      // instead of "(Object Object)".
      console.error('Fetch error:', error);
      let errorText = error.message; // Get the error message string
      if (typeof errorText !== 'string') { // Fallback if error.message isn't a string
      ...
      const errorMessage = {
        id: Date.now() + 1,
        role: 'assistant',
        content: `Something went wrong. Please try again. (${errorText})`
      };
      // --- END OF FIX ---
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex flex-col items-center justify-start min-h-full py-8 px-4 sm:px-6 max-w-5xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mb-10">
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-[#62AADE]/50 transition-all">
                    <div className="flex items-start gap-3">
                      <div className="bg-gradient-to-br from-[#163791] to-[#62AADE] rounded-lg p-2.5">
                        <BookOpen className="w-5 h-5 text-white" />
                      </div>
                      <div>
                        <h3 className="text-white font-semibold mb-1">Learn Guidelines</h3>
                        <p className="text-gray-400 text-sm">Understand evaluation criteria and classification rules</p>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-[#62AADE]/50 transition-all">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-white font-semibold mb-1">Ask Questions</h3>
                        <p className="text-gray-400 text-sm">Get instant answers from all documentation</p>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-[#62AADE]/50 transition-all">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-white font-semibold mb-1">See Examples</h3>
                        <p className="text-gray-400 text-sm">Access real error examples and classifications</p>
                      </div>
                    </div>
                  </div>
                </div>
